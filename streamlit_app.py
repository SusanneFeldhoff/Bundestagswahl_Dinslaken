...
# === Eingaben ===
st.title("Wahlergebnisse Dinslaken 2025")
wahltyp = st.selectbox("Wähle den Typ der Stimme", ["Erststimme", "Zweitstimme"])
darstellung = st.radio("Darstellung", ["Siegerpartei pro Stimmbezirk", "Bester Stimmbezirk pro Partei im Bundestag", "Komplettdarstellung pro Partei im Bundestag"])

# === Dropdown für Komplettdarstellung ===
bundestagsparteien = ["CDU", "SPD", "GRÜNE", "AfD", "Die Linke"]
partei_auswahl = st.selectbox("Wähle eine Partei", bundestagsparteien) if darstellung == "Komplettdarstellung pro Partei im Bundestag" else None
...
if darstellung == "Siegerpartei pro Stimmbezirk":
    ...
    for _, row in gdf_merged.iterrows():
        ...
        popup = folium.Popup(popup_html(row), max_width=2000)
...
# === Komplettdarstellung mit Choropleth ===
    ...
    if spaltenname not in gdf_merged.columns:
        ...
        if partei_auswahl == "CDU":
            colormap = LinearColormap(
                colors=["#aaaaaa", "#555555", "#000000"],  # HELL → DUNKEL
